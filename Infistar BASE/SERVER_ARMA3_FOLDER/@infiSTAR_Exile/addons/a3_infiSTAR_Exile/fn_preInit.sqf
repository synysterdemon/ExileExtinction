...
fn_onInventoryOpened = compileFinal "
	...
	if(!isNil'INV_OPEN_THREAD')then{terminate INV_OPEN_THREAD;INV_OPEN_THREAD=nil;};
	INV_OPEN_THREAD = [] spawn {
		disableSerialization;
		waitUntil {!isNull ((findDisplay 602) displayCtrl 111)};
		waitUntil {
			((findDisplay 602) displayCtrl 111) ctrlSetText format['%1 - %2',groupId(group player),profileName];
			isNull ((findDisplay 602) displayCtrl 111)
		};
	};
	if(!_ret)then
	{
...
if((getPlayerUID _target) isEqualTo '')exitWith{systemChat 'can not revive. not a player anymore.. !';};
...
if((_wall_look)||(_wall_glitch_object)||(_wall_glitch_vehicle))then{
	fnc_check_allowed_build = compileFinal "
		...
		_val
	";
	publicVariable "fnc_check_allowed_build";
};
