...
UPDATEEMAIL='synysterdemon@live.co.uk';
INFISTARVERSION='14-Jul-2016 12-41-09 - v0055';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!   synysterdemon@live.co.uk - 14-Jul-2016 12-41-09 - v0055 - %2 - %3',_t,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp in your MPmission is NOT UPDATED   synysterdemon@live.co.uk - 14-Jul-2016 12-41-09 - v0055 - %2 - %3',_t,serverName,productVersion];
	diag_log format['<infiSTAR.de> %1 - infiSTAR will NOT START if infiSTAR_AdminMenu.hpp is not updated!',_t];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    synysterdemon@live.co.uk - 14-Jul-2016 12-41-09 - v0055 - %2 - %3',_t,serverName,productVersion];
...
FNC_A3_RL = compileFinal "
	...
	_buildurl = format['http://htmlload.infistar.de/rl.php?owner=synysterdemon@live.co.uk&owneruid=76561198031739652&infistarversion=14-Jul-2016 12-41-09 - v0055&servername=%1&logname=%2&logentry=%3',serverName,_logname,_logentry];
...
_fnc_RandomGen =
{
	...
	_fnc_actualGen = {
		...
		_main	= ['d','Q','v','F','p','Y','t','o','V','5','n','x','e','M','d','t','m','9','P','X','d','P','H','N','b','h','f','N','6','H'];
...
} forEach ['ExileIsPlayingRussianRoulette','ExileClientIsHandcuffed'];

player removeAllEventHandlers 'InventoryOpened';
player addEventHandler ['InventoryOpened', { if(ExileIsPlayingRussianRoulette || ExileClientIsHandcuffed)then{true}else{_this call fn_onInventoryOpened} }];
...
if(_isNormal)then{
(findDisplay 46) displayRemoveEventHandler ['KeyDown',_stopvaultglitch_id];
_stopvaultglitch_id = (findDisplay 46) displayAddEventHandler ['KeyDown', _fnc_stopvaultglitch];	
};
	...
	if(_unit isEqualTo player)then
	{
		...
		
		{
			if(_x isKindOf 'Exile_Construction_Abstract_Static')exitWith
			{
				...
				['ErrorTitleOnly', [format['You just glitched through %1.. be careful!',typeOf _x]]] call ExileClient_gui_toaster_addTemplateToast;
			};
		} forEach _lineintersectsobjs;
	};
};
...
[
	[_admins],
	{
		...
		_devLog = format['infiSTAR.de AHAT %1 - 14-Jul-2016 12-41-09 - v0055 - server running: %2:2520 - AdminNameTag: "+str _AdminNameTag+"',VERSIONCHECKRESULT,call _GET_TIME_TIME];diag_log _devLog;
		"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
...
_from = 'synysterdemon@live.co.uk';
_vers = '14-Jul-2016 12-41-09 - v0055';
...
"; }; _A3AHstring = _A3AHstring + "
			...
			{
				_RATING_CHECK_LOOPS_1 = _x getVariable['"+_RATING_CHECK_LOOPS+"_1',0];
				if(_RATING_CHECK_LOOPS_1 > 1)then
				{
					if(_x getVariable['isrunning',false])then
					{
						_x setVariable['isrunning',false,true];
						_x setVariable['"+_RATING_CHECK_LOOPS+"_1',0];
					}
					else
					{
				...
				{
					_x setVariable['"+_RATING_CHECK_LOOPS+"_1',_RATING_CHECK_LOOPS_1 + 1];
				};



				_RATING_CHECK_LOOPS_2 = _x getVariable['"+_RATING_CHECK_LOOPS+"_2',0];
				if(_RATING_CHECK_LOOPS_2 > 4)then
				{
					_rating = rating _x;
					...
					_x setVariable['"+_RATING_CHECK_LOOPS+"_2',0];
				}
				else
				{
					_x setVariable['"+_RATING_CHECK_LOOPS+"_2',_RATING_CHECK_LOOPS_2 + 1];
				};
